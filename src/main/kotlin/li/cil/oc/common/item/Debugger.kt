...
class Debugger(override val parent: Delegator) : Delegate {
    ...
    override fun onItemUse(stack: ItemStack, player: EntityPlayer, position: BlockPosition, side: EnumFacing, hitX: Float, hitY: Float, hitZ: Float): Boolean {
        ...
        return when (tileEntity) {
            is SidedEnvironment -> {
                if (!world.isRemote) {
                    DebuggerObject.reconnect(arrayOf(tileEntity.sidedNode(side)))
                }
                true
            }
            is Environment -> {
                if (!world.isRemote) {
                    DebuggerObject.reconnect(arrayOf(tileEntity.node()))
                }
                true
            }
            else -> {
