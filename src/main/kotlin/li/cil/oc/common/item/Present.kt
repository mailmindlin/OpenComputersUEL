...
class Present(override val parent: Delegator) : Delegate {
    init {
    ...
    override fun onItemRightClick(stack: ItemStack, world: World, player: EntityPlayer): ActionResult<ItemStack> {
        if (stack.count > 0) {
            ...
            if (!world.isRemote) {
                ...
                val present = PresentObject.nextPresent()
...
object PresentObject {
    private val presents: Array<ItemStack> by lazy {
        ...
        fun add(name: String, weight: Int) {
            ...
            if (item != null) {
                ...
                if (ItemUtils.getIngredients(stack).isNotEmpty()) {
                    repeat(weight) { result.add(stack) }
                }
            } else {
    ...
    @JvmStatic
    fun nextPresent(): ItemStack = presents[rng.nextInt(presents.size)].copy()
}
