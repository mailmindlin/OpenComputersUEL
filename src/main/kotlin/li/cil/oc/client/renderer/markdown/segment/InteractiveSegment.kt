 ...
 */
interface InteractiveSegment : Segment {
    /**
     ...
     */
    fun onMouseClick(mouseX: Int, mouseY: Int): Boolean = false
    ...
    fun checkHovered(mouseX: Int, mouseY: Int, x: Int, y: Int, w: Int, h: Int): InteractiveSegment? =
        if (mouseX >= x && mouseY >= y && mouseX <= x + w && mouseY <= y + h) this else null
}
